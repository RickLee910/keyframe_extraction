...
if __name__ == "__main__":
    ...
    frame_diffs = []
    frames = []
    success, frame = cap.read() #按帧读取视频，success:按帧读取正确true 读到结尾falsee frame:每一帧图像，三维矩阵
    i = 0
    while (success):
        ...
        if curr_frame is not None and prev_frame is not None:
            # logic here
            diff = cv2.absdiff(curr_frame, prev_frame)#两幅图像做差
            ...
            #prevent memory overflow caused by too large video
            # if i % 1000 == 0:
            #     frame_diffs.clear()
            #     frames.clear()
